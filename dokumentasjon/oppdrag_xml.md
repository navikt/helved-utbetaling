...
| saksbehId             | NAV-ident. Maks 8 tegn i simulering. I utbetalingsoppdrag kan det være flere tegn.                                            | NAV-ident på saksbehandleren som har gjort behandlingen i vedtaksløsningen. Denne oppdaterer vi for hver nye utbetaling på saken                                                                                   | 
...
| datoStatusFom     | Dato                              | Optional felt som vi bare bruker dersom `kodeStatusLinje` også er satt. Vi bruker disse feltene bare ved opphør. `kodeStatusFom` sier når `kodeStatusLinje` skal gjelde fra. Altså når opphøret skal gjelde fra.
...
| henvisning       | Maks 30 tegn string                 | Henvisning brukes for å spore hver enkelt utbetalingsperiode tilbake til hendelsen i vedtaksløsningen som trigget utbetalingen. Vi setter behandlingId i dette feltet, men kunne evt. brukt noe annet som feks vedtakId, så lenge id-en er granulær nok. Det hadde mao ikke vært OK å sette sakId i dette feltet, fordi det finnes flere hendelser/behandlinger/vedtak på samme sak.<br/><br/>Tilleggsstønader gjenbruker i noen tilfeller behandlingId til flere utbetalinger på en sak. Det kan for eksempel skje dersom de har innvilget flere månedskort fremover i tid. Da utbetales ikke alle med en gang. De utbetales etter hvert som nye 30-dagersperioder starter.                                                                                                                                                                                          | 
| Attestant180     | NAV-ident, wrappes i et objekt. Maks 8 tegn i simulering. I utbetalingsoppdrag kan det være flere tegn.      | Vi setter beslutter på vedtaket i dette feltet.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
