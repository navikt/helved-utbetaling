...
class SuppressProcessor<K: Any, V: Any>(
        ...
        fun <K: Any, V: Any> supplier(
            ...
            return object: ProcessorSupplier<Windowed<K>, List<StreamsPair<V, V?>>, K, List<StreamsPair<V, V?>>> {
                ...
                override fun stores(): Set<StoreBuilder<*>> {
                    ...
                    return setOf(org.apache.kafka.streams.state.Stores.timestampedKeyValueStoreWithHeadersBuilder(inner, store.serde.key, store.serde.value))
                }
    ...
    private lateinit var bufferStore: TimestampedKeyValueStoreWithHeaders<Windowed<K>, List<StreamsPair<V, V?>>>
    ...
    override fun init(ctx: ProcessorContext<K, List<StreamsPair<V, V?>>>) {
        context = ctx
        bufferStore = context.getStateStore(stateStoreName) as TimestampedKeyValueStoreWithHeaders<Windowed<K>, List<StreamsPair<V, V?>>>
    ...
    override fun process(record: org.apache.kafka.streams.processor.api.Record<Windowed<K>, List<StreamsPair<V, V?>>>) {
        bufferStore.put(record.key(), ValueTimestampHeaders.make(record.value(), record.timestamp(), record.headers()))
    }
    ...
    private fun punctuate(wallClockTime: Long) {
        ...
        while (iterator.hasNext()) {
            ...
            if (wallClockTime > emissionTimeThreshold) {
                ...
                context.forward(org.apache.kafka.streams.processor.api.Record(windowedKey.key(), latestAggregate, validTimestamp, valueAndTimestamp.headers()))
