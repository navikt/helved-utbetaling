...
class SnickerboaApiTest {
    ...
    fun `health endpoint returns OK`() {
        val response = TestRuntime.app.handler(Request(Method.GET, "/actuator/live"))
        assertEquals(Status.OK, response.status)

        val ready = TestRuntime.app.handler(Request(Method.GET, "/actuator/ready"))
        assertEquals(Status.OK, ready.status)
    }

    @Test
    fun `aap utbetaling produces to kafka and returns OK on status reply`() = runBlocking {
        val responseDeferred = async(Dispatchers.IO) {
            TestRuntime.app.handler(
                Request(Method.POST, "/abetal/aap")
                    .with(aapLens of aapUtbetaling())
            )
        }
        ...
        assertEquals(Status.OK, responseDeferred.await().status)
    }
    ...
    fun `aap dryrun produces to kafka and returns simulering`() = runBlocking {
        val responseDeferred = async(Dispatchers.IO) {
            TestRuntime.app.handler(
                Request(Method.POST, "/abetal/aap")
                    .with(aapLens of aapUtbetaling(dryrun = true))
            )
        }
    ...
    fun `aap utbetaling returns error status when status reply is feilet`() = runBlocking {
        ...
        assertEquals(Status.UNPROCESSABLE_ENTITY, responseDeferred.await().status)
    }
}

private suspend fun <V> awaitProduced(producer: libs.kafka.KafkaProducerFake<String, V>): String =
    withTimeout(5.seconds) {
        while (producer.history().isEmpty()) delay(10.milliseconds)
