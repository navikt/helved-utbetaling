...
object OppdragService {
    ...
    fun opprett(new: UtbetalingV2, erførsteUtbetalingPåSak: Boolean): Oppdrag {
        val oppdrag110 = objectFactory.createOppdrag110().apply {
            kodeAksjon = "1"
            kodeEndring = if (erførsteUtbetalingPåSak) "NY" else "ENDR"
            kodeFagomraade = new.fagsystem().kode
            fagsystemId = new.sakId.id
            utbetFrekvens = "MND"
            oppdragGjelderId = new.personident.ident
            datoOppdragGjelderFom = LocalDate.of(2000, 1, 1).toXMLDate()
            saksbehId = new.saksbehandlerId.ident
            avstemming115 = avstemming115(new.fagsystem().kode)
            new.avvent?.let { avvent118 = avvent118(it) }
            oppdragsEnhet120s.addAll(oppdragsEnhet120(new))
            addLinjer(new)
        }
    ...
    private fun Oppdrag110.addLinjer(utbetaling: UtbetalingV2) {
        utbetaling.kjeder.values.forEach { value ->
            var forrigeId: String? = null
            value.perioder.forEachIndexed { i, periode ->
                val periodeId =
                    if (i == value.perioder.size - 1) value.lastPeriodeIdOrNew() else UUID.randomUUID().toString()
                val oppdragslinje = oppdragsLinje150(
                    utbetaling = utbetaling,
                    stønadstype = value.stønad,
                    satstype = value.satstype,
                    erEndringPåEksisterendePeriode = false,
                    periode = periode,
                    periodeId = periodeId,
                    forrigePeriodeId = forrigeId.also { forrigeId = periodeId },
    ...
    private fun UtbetalingV2.Utbetalingsperioder.lastPeriodeIdOrNew(): String {
        return lastPeriodeId ?: UUID.randomUUID().toString()
    }
    ...
    // før denne kalles, join prev med status for å sjekke om den er locket (status != OK)
    fun update(new: UtbetalingV2, prev: UtbetalingV2): Oppdrag {
        prev.validateLockedFields(new)
        prev.validateMinimumChanges(new)
        ...
        val oppdrag110 = objectFactory.createOppdrag110().apply {
            ...
            new.avvent?.let { avvent118 = avvent118(it) }
            oppdragsEnhet120s.addAll(oppdragsEnhet120(new))
            new.kjeder.entries.forEach { (key, value) ->
                val prevKjede = prev.kjeder[key]
                if (prevKjede == null) {
                    // Har ikke tidligere kjede, opprett en ny
                    var forrigeId: String? = null
                    value.perioder.forEachIndexed { i, periode ->
                        val periodeId =
                            if (i == value.perioder.size - 1) value.lastPeriodeIdOrNew() else UUID.randomUUID().toString()
                    ...
                    // Har tidligere kjede, kjed de nye periodene sammen med de gamle
                    val prevPeriods = prevKjede.perioder.sortedBy { it.fom } // ensure its sorted
                    val newPeriods = value.perioder.sortedBy { it.fom } // ensure its sorted

                    val opphørsdato = opphørsdatoV2(newPeriods, prevPeriods, value.satstype)
                    if (opphørsdato != null) {
                        val opphørslinje = oppdragsLinje150(
                            utbetaling = new,
                            satstype = value.satstype,
                            stønadstype = value.stønad,
                            erEndringPåEksisterendePeriode = true,
                            periode = prevPeriods.last(),
                            periodeId = prevKjede.lastPeriodeIdOrNew(),
                            forrigePeriodeId = null,
                            opphør = opphørsdato
                        )
                        oppdragsLinje150s.add(opphørslinje)
                    }
                    oppdragsLinje150s.addAll(nyeLinjer(new, value, prevKjede, opphørsdato))
                }
            }
        }
        return objectFactory.createOppdrag().apply {
...
fun opphørsdatoV2(
    new: List<UtbetalingV2.Utbetalingsperiode>,
    prev: List<UtbetalingV2.Utbetalingsperiode>,
    satstype: Satstype,
): LocalDate? {
    if (new.first().fom > prev.first().fom) {
        // Forkorter fom i starten. Må opphøre fra start
...
private fun nyeLinjer(
    ...
    new: UtbetalingV2.Utbetalingsperioder,
    prev: UtbetalingV2.Utbetalingsperioder,
    opphørsdato: LocalDate?,
): List<OppdragsLinje150> {
    var førsteEndringIdx = prev.perioder.zip(new.perioder).indexOfFirst { it.first != it.second }
    ...
    return new.perioder
        ...
        .map { p ->
            ...
            oppdragsLinje150(utbetaling, new.satstype, new.stønad, false, p, pid, sistePeriodeId, null).also {
                sistePeriodeId = pid
            }
        }
}
...
private fun oppdragsLinje150(
    ...
    erEndringPåEksisterendePeriode: Boolean,
    periode: UtbetalingV2.Utbetalingsperiode,
    periodeId: String,
    forrigePeriodeId: String?,
    opphør: LocalDate?,
): OppdragsLinje150 {
    val attestant = objectFactory.createAttestant180().apply {
        attestantId = utbetaling.beslutterId.ident
    ...
    return objectFactory.createOppdragsLinje150().apply {
        ...
        forrigePeriodeId?.let {
            refDelytelseId = forrigePeriodeId
            refFagsystemId = utbetaling.sakId.id
        }
        vedtakId = utbetaling.vedtakstidspunkt.toLocalDate().toString()
        delytelseId = periodeId
        kodeKlassifik = stønadstype.klassekode
        datoVedtakFom = periode.fom.toXMLDate()
        datoVedtakTom = periode.tom.toXMLDate()
        sats = BigDecimal.valueOf(periode.beløp.toLong())
        fradragTillegg = TfradragTillegg.T
        typeSats = satstype.kode
        brukKjoreplan = "N"
        saksbehId = utbetaling.saksbehandlerId.ident
        utbetalesTilId = utbetaling.personident.ident
        henvisning = utbetaling.behandlingId.id
...
private fun oppdragsEnhet120(new: UtbetalingV2): List<OppdragsEnhet120> {
    val bos = objectFactory.createOppdragsEnhet120().apply {
        enhet = new.betalendeEnhet()?.enhet ?: "8020"
        typeEnhet = "BOS"
        datoEnhetFom = LocalDate.of(1970, 1, 1).toXMLDate()
    }
    val beh = objectFactory.createOppdragsEnhet120().apply {
    ...
    return when (new.betalendeEnhet()) {
        null -> listOf(bos)
        else -> listOf(bos, beh)
    }
}
