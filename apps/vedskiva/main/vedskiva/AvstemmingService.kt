...
class AvstemmingService(
    ...
    suspend fun generate(
        ...
        peisschtappern.oppdragPaged(fom = avstemFom, tom = avstemTom) { page ->
            fetched += page.size
            page.filter { dao ->
                ...
                if (dao.value == null) {
                    appLog.error("Found tombstone key:${dao.key} p:${dao.partition} o:${dao.offset}")
                } else {
        ...
        appLog.info("Fetched $fetched oppdrag between $avstemFom - $avstemTom, keeping $kept")
